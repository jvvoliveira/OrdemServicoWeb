...
public class ServicoServico extends ServicoS<Servico> {
    ...
    public boolean existe(@NotNull Servico servico) {
        TypedQuery<Servico> query
                = entityManager.createNamedQuery(Servico.SERVICO_POR_CLIENTE_CPF, classe);
        query.setParameter(1, servico.getId());
        return !query.getResultList().isEmpty();
    }
    
    @TransactionAttribute(SUPPORTS)
    public List<Servico> consultarPorClienteCPF(@NotNull String cpf) {
    ...
    @TransactionAttribute(SUPPORTS)
    public List<Servico> consultarPorStatus(@NotNull String status) {
